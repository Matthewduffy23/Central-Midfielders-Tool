...
# ----------------- METRIC LEADERBOARD (final blue) -----------------
import re, numpy as np, matplotlib.pyplot as plt
from matplotlib.colors import LinearSegmentedColormap, Normalize
from matplotlib.ticker import FuncFormatter
from matplotlib import rcParams, font_manager as fm

# --- Crisp rendering & typography
rcParams.update({
    "figure.dpi": 280,
    "savefig.dpi": 280,
...
# (Optional) ship Inter for max crispness; auto-register if present
...
# Minimal settings (no palette toggles)
...
# ----- Fixed palette: dark blue → light blue (darker = better)
cmap = LinearSegmentedColormap.from_list(
    "deep_to_light_blue",
    ["#0a2a66", "#1e4fb3", "#3d7af0", "#8fbbff", "#dcecff"]  # smooth, subtle fade
)
...
# extra left margin for labels; right for value breathing room
plt.subplots_adjust(top=0.90, left=0.27, right=0.965, bottom=0.14)

# Bars (no edges for sharpness)
bars = ax.barh(range(len(vals)), vals, color=colors, edgecolor="none", zorder=2)

# Axes & labels
ax.invert_yaxis()
ax.set_yticks(range(len(vals)))
ax.set_yticklabels(y_labels, fontsize=10.5, color="#0f172a")  # darker label text
ax.set_ylabel("")
ax.set_xlabel(val_col, color="#111827", labelpad=6, fontsize=10.0)  # 1 size smaller
# Very light gridlines for structure
ax.grid(axis="x", color="#e9ebef", linewidth=0.8, zorder=1)

# Remove left ticks/line; keep bottom subtle; hide top/right
...
# Value labels: small, plain, black, at right edge with consistent padding
